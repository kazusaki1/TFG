...
def main(image_name):
	...
	img = Image.open(os.getcwd()+"\\uploaded_media\\"+image_name)
	wpercent = (80 / float(img.size[0]))
	hpercent = (80 / float(img.size[1]))
	img = img.resize((80, 80), PIL.Image.ANTIALIAS)
	img.save(os.getcwd()+"\\uploaded_media\\"+image_name)


	#here we open the image
	image = misc.imread(os.getcwd()+"\\uploaded_media\\"+image_name)
